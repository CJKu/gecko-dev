  ...
  case eCSSUnit_ListDep: {
         ...
         item; item = item->mNext)
    {
      if (item->mValue.UnitHasStringValue()) {
        ...
        if (prop != eCSSProperty_UNKNOWN) {
          // If the property given is a shorthand, it indicates the
          // expectation for all the longhands the shorthand
          // expands to.
                                  ...
                                  const RuleNodeCacheConditions aConditions)
{
  ...
  if (rebuild) {
    ...
    FillBackgroundList(bg->Layers(), &nsStyleImageLayers::Layer::mImage,
                       bg->mLayers.mImageCount, fillCount);
    FillBackgroundList(bg->Layers(), &nsStyleImageLayers::Layer::mRepeat,
                       bg->mLayers.mRepeatCount, fillCount);
    FillBackgroundList(bg->Layers(), &nsStyleImageLayers::Layer::mAttachment,
                       bg->mLayers.mAttachmentCount, fillCount);
    FillBackgroundList(bg->Layers(), &nsStyleImageLayers::Layer::mClip,
                       bg->mLayers.mClipCount, fillCount);
    FillBackgroundList(bg->Layers(), &nsStyleImageLayers::Layer::mBlendMode,
                       bg->mLayers.mBlendModeCount, fillCount);
    FillBackgroundList(bg->Layers(), &nsStyleImageLayers::Layer::mOrigin,
                       bg->mLayers.mOriginCount, fillCount);
    FillBackgroundList(bg->Layers(), &nsStyleImageLayers::Layer::mPosition,
                       bg->mLayers.mPositionCount, fillCount);
    FillBackgroundList(bg->Layers(), &nsStyleImageLayers::Layer::mSize,
