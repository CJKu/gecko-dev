                                              ...
                                              LayerManager *aLayerManager)
{
  ...
  // Keep move forward event if svgMaskFrame is nullptr or isOK is false.
  // Since even if svgReset->mMask is not valid, we still have svg->mImages.
  ...
  if (opacity != 1.0f ||  (clipPathFrame && !isTrivialClip)
    ...
    if (svgMaskFrame) {
      ...
      Point deviceOffset = target->GetDeviceOffset();
