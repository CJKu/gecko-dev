...
nsSVGIntegrationUtils::UsingEffectsForFrame(const nsIFrame* aFrame)
{
  ...
  // TBD: temporary put it here.
  const nsStyleMask *mask = aFrame->StyleMask();
  return (style->HasFilters() ||
          style->mClipPath.GetType() != NS_STYLE_CLIP_PATH_NONE || style->mMask || (mask->mLayers.mImageCount != 0));
}
                                              ...
                                              LayerManager *aLayerManager)
{
  ...
  const nsStyleMask *mask = firstFrame->StyleMask();
  ...
  if (mask->mLayers.mImageCount != 0) {
    aContext.SetMatrix(matrixAutoSaveRestore.Matrix());
    nsRenderingContext rc(&aContext);

    //  Paint all mask images onto a SourceSurface.
    aContext.PushGroup(gfxContentType::COLOR_ALPHA);
    ...
    // Blend this mask surface with current source surface onto the destination.
