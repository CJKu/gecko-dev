...
nsImageRenderer::ComputeObjectAnchorPoint(
  const nsStyleLayers::Position& aPos,
...
/* static */ void
nsCSSRendering::GetLayerClip(const nsStyleLayers::Layer& aLayer,
                             ...
                             /* out */ LayerClipState* aClipState)
{
  ...
  if (isSolidBorder && backgroundClip == NS_STYLE_LAYER_CLIP_BORDER) {
    // If we have rounded corners, we need to inflate the background
    // drawing area a bit to avoid seams between the border and
    // background.
    backgroundClip = haveRoundedCorners ?
      NS_STYLE_LAYER_CLIP_MOZ_ALMOST_PADDING : NS_STYLE_LAYER_CLIP_PADDING;
  }
  ...
  if (aForFrame->GetType() == nsGkAtoms::scrollFrame &&
    ...
    if (backgroundClip == NS_STYLE_LAYER_CLIP_CONTENT) {
    ...
    backgroundClip = NS_STYLE_LAYER_CLIP_PADDING;
  }

  if (backgroundClip != NS_STYLE_LAYER_CLIP_BORDER) {
    nsMargin border = aForFrame->GetUsedBorder();
    if (backgroundClip == NS_STYLE_LAYER_CLIP_MOZ_ALMOST_PADDING) {
    ...
    } else if (backgroundClip != NS_STYLE_LAYER_CLIP_PADDING) {
      NS_ASSERTION(backgroundClip == NS_STYLE_LAYER_CLIP_CONTENT,
...
static void
SetupLayerClip(nsCSSRendering::LayerClipState& aClipState,
...
static void
DrawBackgroundColor(nsCSSRendering::LayerClipState& aClipState,
                    gfxContext *aCtx, nscoord aAppUnitsPerPixel)
{
  if (aClipState.mDirtyRectGfx.IsEmpty()) {
    // Our caller won't draw anything under this condition, so no need
                                         ...
                                         bool& aDrawBackgroundColor)
{
  ...
  if (aDrawBackgroundColor &&
      bg->BottomLayer().mRepeat.mXRepeat == NS_STYLE_LAYER_REPEAT_REPEAT &&
      bg->BottomLayer().mRepeat.mYRepeat == NS_STYLE_LAYER_REPEAT_REPEAT &&
                                      ...
                                      bool aMask)
{
  ...
  const nsStyleLayers &layers = aMask ? aBackgroundSC->StyleSVGReset()->mLayers :
  ...
  if (aBGClipRect) {
    ...
    GetLayerClip(layers.BottomLayer(),
  ...
  if (aBackgroundSC != aForFrame->StyleContext()) {
    NS_FOR_VISIBLE_CSS_LAYERS_BACK_TO_FRONT_WITH_RANGE(i, layers, startLayer, nLayers) {
  ...
  if (drawBackgroundImage) {
    bool clipSet = false;
    uint8_t currentBackgroundClip = NS_STYLE_LAYER_CLIP_BORDER;
    NS_FOR_VISIBLE_CSS_LAYERS_BACK_TO_FRONT_WITH_RANGE(i, layers, layers.mImageCount - 1,
                                                              nLayers + (layers.mImageCount -
                                                                         startLayer - 1)) {
      const nsStyleLayers::Layer &layer = layers[i];
      if (!aBGClipRect) {
        if (currentBackgroundClip != layer.mClip || !clipSet) {
          ...
          // already called GetLayerClip above and it stored its results
          // in clipState.
          if (clipSet) {
            autoSR.Restore(); // reset the previous one
            GetLayerClip(layer, aForFrame,
                              aBorder, aBorderArea, aDirtyRect, (aFlags & PAINTBG_WILL_PAINT_BORDER),
                              appUnitsPerPixel, &clipState);
          }
          SetupLayerClip(clipState, ctx, appUnitsPerPixel, &autoSR);
          clipSet = true;
      ...
      if ((aLayer < 0 || i == (uint32_t)startLayer) &&
        ...
        nsBackgroundLayerState state = PrepareLayer(aPresContext, aForFrame,
...
nsRect
nsCSSRendering::ComputeLayerPositioningArea(nsPresContext* aPresContext,
                                                 nsIFrame* aForFrame,
                                                 const nsRect& aBorderArea,
                                                 ...
                                                 nsIFrame** aAttachedToFrame)
{
  ...
  if (MOZ_UNLIKELY(frameType == nsGkAtoms::scrollFrame &&
    ...
    if (aLayer.mOrigin == NS_STYLE_LAYER_ORIGIN_BORDER) {
      nsMargin border = geometryFrame->GetUsedBorder();
      border.ApplySkipSides(geometryFrame->GetSkipSides());
      bgPositioningArea.Inflate(border);
      bgPositioningArea.Inflate(scrollableFrame->GetActualScrollbarSizes());
    } else if (aLayer.mOrigin != NS_STYLE_LAYER_ORIGIN_PADDING) {
      nsMargin padding = geometryFrame->GetUsedPadding();
      padding.ApplySkipSides(geometryFrame->GetSkipSides());
      bgPositioningArea.Deflate(padding);
      NS_ASSERTION(aLayer.mOrigin == NS_STYLE_LAYER_ORIGIN_CONTENT,
  ...
  if (aLayer.mOrigin != NS_STYLE_LAYER_ORIGIN_BORDER && geometryFrame) {
    nsMargin border = geometryFrame->GetUsedBorder();
    if (aLayer.mOrigin != NS_STYLE_LAYER_ORIGIN_PADDING) {
  ...
  if (NS_STYLE_LAYER_ATTACHMENT_FIXED == aLayer.mAttachment) {
...
ComputeDrawnSizeForBackground(const CSSSizeOrRatio& aIntrinsicSize,
                              ...
                              const nsStyleLayers::Size& aLayerSize)
{
  // Size is dictated by cover or contain rules.
  if (aLayerSize.mWidthType == nsStyleLayers::Size::eContain ||
      aLayerSize.mWidthType == nsStyleLayers::Size::eCover) {
  ...
  if (aLayerSize.mWidthType == nsStyleLayers::Size::eLengthPercentage) {
    specifiedSize.SetWidth(
      aLayerSize.ResolveWidthLengthPercentage(aBgPositioningArea));
  }
  if (aLayerSize.mHeightType == nsStyleLayers::Size::eLengthPercentage) {
...
nsCSSRendering::PrepareLayer(nsPresContext* aPresContext,
                                       nsIFrame* aForFrame,
                                       uint32_t aFlags,
                                       const nsRect& aBorderArea,
                                       ...
                                       bool aMask)
{
  ...
  // it to compute the effective image size for a CSS gradient.
  nsRect bgPositioningArea =
    ComputeLayerPositioningArea(aPresContext, aForFrame, aBorderArea,
  ...
  if (repeatX == NS_STYLE_LAYER_REPEAT_REPEAT) {
    state.mFillArea.x = bgClipRect.x;
    state.mFillArea.width = bgClipRect.width;
  }
  if (repeatY == NS_STYLE_LAYER_REPEAT_REPEAT) {
                                       ...
                                       uint32_t aFlags)
{
  ...
  nsBackgroundLayerState state =
      PrepareLayer(aPresContext, aForFrame, aFlags, borderArea,
                             aClipRect, aLayer);
...
nsImageRenderer::ComputeIntrinsicSize()
{
  ...
  switch (mType) {
    ...
    case eStyleImageType_Element:
    {
      ...
      //     Make sure to change nsStyleLayers::Size::DependsOnFrameSize
      //     when fixing this!
      if (mPaintServerFrame) {
