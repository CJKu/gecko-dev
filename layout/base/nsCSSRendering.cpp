...
/* static */ void
nsCSSRendering::GetImageLayersClip(const nsStyleImageLayers::Layer& aLayer,
                                  nsIFrame* aForFrame,
                                  ...
                                  nscoord aAppUnitsPerPixel,
                                  /* out */ ImageLayersClipState* aClipState)
{
  // Compute the outermost boundary of the area that might be painted.
...
static void
DrawBackgroundColor(nsCSSRendering::ImageLayersClipState& aClipState,
                    gfxContext *aCtx, nscoord aAppUnitsPerPixel)
{
  if (aClipState.mDirtyRectGfx.IsEmpty()) {
    // Our caller won't draw anything under this condition, so no need
                                      ...
                                      int32_t aLayer)
{
  ...
  ImageLayersClipState clipState;
  if (aBGClipRect) {
  ...
  if (drawBackgroundImage) {
    ...
    NS_FOR_VISIBLE_IMAGELAYER_BACK_TO_FRONT_WITH_RANGE(i, layers,
      ...
      if (!aBGClipRect) {
        if (currentBackgroundClip != layer.mClip || !clipSet) {
          ...
          // already called GetImageLayersClip above and it stored its results
          // in clipState.
          if (clipSet) {
            autoSR.Restore(); // reset the previous one
            GetImageLayersClip(layer, aForFrame,
